...
namespace jank::analyze
{
                        ...
                        bool needs_box)
  {
     ...
     * conversions above. */
    auto final_then_type{ then_expr.expect_ok()->get_type() };
    auto final_else_type{ else_expr_opt.is_some() ? else_expr_opt.unwrap()->get_type()
                                                  : cpp_util::untyped_object_ref_type() };
    if(cpp_util::is_typed_object(final_then_type) && cpp_util::is_any_object(final_else_type)
       && Cpp::GetCanonicalType(final_then_type) != Cpp::GetCanonicalType(final_else_type))
    {
      final_then_type = cpp_util::untyped_object_ref_type();
    }
