...
namespace jank::analyze::expr
{
  ...
  void cpp_unsafe_cast::propagate_position(expression_position const pos)
  {
    position = pos;
  }

  object_ref cpp_unsafe_cast::to_runtime_data() const
  {
    /* TODO: Fill in. */
    return merge(expression::to_runtime_data(), obj::persistent_array_map::create_unique());
  }

  void cpp_unsafe_cast::walk(std::function<void(jtl::ref<expression>)> const &f)
  {
    f(value_expr);
    expression::walk(f);
  }
