...
{
  std::string Error;
  llvm::Target const *TheTarget = llvm::TargetRegistry::lookupTarget(TargetStr, Error);
  if(!TheTarget)
  {
    llvm::errs() << Error;
    return 1;
...
static int PrintSupportedExtensions(std::string TargetStr)
{
  ...
  llvm::Triple const &MachineTriple = TheTargetMachine->getTargetTriple();
  llvm::MCSubtargetInfo const *MCInfo = TheTargetMachine->getMCSubtargetInfo();
  llvm::ArrayRef<llvm::SubtargetFeatureKV> const Features = MCInfo->getAllProcessorFeatures();

  llvm::StringMap<llvm::StringRef> DescMap;
  for(llvm::SubtargetFeatureKV const &feature : Features)
  {
...
static int PrintEnabledExtensions(TargetOptions const &TargetOpts)
{
  std::string Error;
  llvm::Target const *TheTarget = llvm::TargetRegistry::lookupTarget(TargetOpts.Triple, Error);
  if(!TheTarget)
  {
    llvm::errs() << Error;
    return 1;
  ...
  std::unique_ptr<llvm::TargetMachine> TheTargetMachine(
    TheTarget->createTargetMachine(TargetOpts.Triple,
                                   TargetOpts.CPU,
                                   FeaturesStr,
                                   BackendOptions,
                                   std::nullopt));
  ...
  std::vector<llvm::SubtargetFeatureKV> const Features = MCInfo->getEnabledProcessorFeatures();
...
int cc1_main(ArrayRef<char const *> Argv, char const *Argv0, void *MainAddr)
{
  ...
  if(llvm::timeTraceProfilerEnabled())
  {
    ...
    if(!Clang->hasFileManager())
    {
      Clang->createFileManager(
        createVFSFromCompilerInvocation(Clang->getInvocation(), Clang->getDiagnostics()));
    }

    if(auto profilerOutput = Clang->createOutputFile(Clang->getFrontendOpts().TimeTracePath,
                                                     /*Binary=*/false,
