...
namespace
{
  ...
  public:
    ...
    static bool
    CreateFromArgs(AssemblerInvocation &Res, ArrayRef<char const *> Argv, DiagnosticsEngine &Diags);
  };

}

bool AssemblerInvocation::CreateFromArgs(AssemblerInvocation &Opts,
                                         ArrayRef<char const *> Argv,
                                         DiagnosticsEngine &Diags)
{
  bool Success = true;

  // Parse the arguments.
  OptTable const &OptTbl = getDriverOptTable();

  llvm::opt::Visibility VisibilityMask(options::CC1AsOption);
  unsigned MissingArgIndex, MissingArgCount;
  InputArgList Args = OptTbl.ParseArgs(Argv, MissingArgIndex, MissingArgCount, VisibilityMask);
  ...
  for(Arg const *A : Args.filtered(OPT_UNKNOWN))
  {
  ...
  if(Arg const *A = Args.getLastArg(OPT_compress_debug_sections_EQ))
  {
  ...
  if(Arg const *A = Args.getLastArg(options::OPT_ffile_compilation_dir_EQ,
  ...
  for(auto const &Arg : Args.getAllArgValues(OPT_fdebug_prefix_map_EQ))
  {
  ...
  {
    bool First = true;
    for(Arg const *A : Args.filtered(OPT_INPUT))
    {
      if(First)
      {
  ...
  Opts.EmitCompactUnwindNonCanonical = Args.hasArg(OPT_femit_compact_unwind_non_canonical);
...
getOutputStream(StringRef Path, DiagnosticsEngine &Diags, bool Binary)
{
  ...
  std::error_code EC;
  auto Out
    = std::make_unique<raw_fd_ostream>(Path,
                                       EC,
...
static bool ExecuteAssemblerImpl(AssemblerInvocation &Opts, DiagnosticsEngine &Diags)
{
  ...
  std::string Error;
  Target const *TheTarget = TargetRegistry::lookupTarget(Opts.Triple, Error);
  if(!TheTarget)
  {
  ...
  {
    for(auto const &KV : Opts.DebugPrefixMap)
    {
  ...
  {
    MCInstPrinter *IP = TheTarget->createMCInstPrinter(llvm::Triple(Opts.Triple),
  ...
  {
    MCSection *AsmLabel
      = Ctx.getMachOSection("__LLVM", "__asm", MachO::S_REGULAR, 4, SectionKind::getReadOnly());
...
int cc1as_main(ArrayRef<char const *> Argv, char const *Argv0, void *MainAddr)
{
