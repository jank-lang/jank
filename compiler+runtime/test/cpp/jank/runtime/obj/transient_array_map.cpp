...
{
  TEST_SUITE("transient_array_map")
  {
    TEST_CASE("promotion to transient_hash_map in assoc_in_place")
    {
      detail::native_array_map data{};
      data.reserve(8);
      ...
      auto const v{ make_box<transient_array_map>(data) };

      CHECK(v->obj_type == object_type::transient_array_map);
