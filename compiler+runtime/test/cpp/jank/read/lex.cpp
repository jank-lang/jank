...
namespace jank::read::lex
{
  ...
  template <size_t N>
  static bool
  operator==(native_vector<jtl::result<token, error_ref>> const &v, std::array<token, N> const &a)
  {
    if(v.size() != N)
    {
      return false;
  ...
  template <size_t N>
  static bool operator==(native_vector<jtl::result<token, error_ref>> const &v,
                         std::array<jtl::result<token, error_ref>, N> const &a)
  {
    if(v.size() != N)
    {
      return false;
  ...
  static error_ref make_error(error::kind const kind, usize const offset, usize const width)
  {
    return runtime::make_box<error::base>(
      ...
      read::source{
        read::no_source_path,
        runtime::__rt_ctx->current_ns_var->deref().to_code_string(),
        {         offset, 1,         offset + 1 },
  ...
  TEST_SUITE("lex")
  {
    ...
    TEST_CASE("Integer")
    {
      ...
      SUBCASE("Arbitrary radix edge cases")
      {
        ...
        native_vector<jtl::result<token, error_ref>> const tokens2(p2.begin(), p2.end());
        CHECK(tokens2
              == make_tokens({
                { 0,
    ...
    TEST_CASE("Character")
    {
      SUBCASE("Whitespace")
      {
        SUBCASE("ASCII")
        {
          ...
          CHECK(tokens
                == make_tokens({
                  ...
                  {  7, 8, token_kind::character,  "\\newline"sv },
                  { 16, 2, token_kind::character,        "\\"sv },
                  { 19, 9, token_kind::character, "\\formfeed"sv },
                  { 29, 7, token_kind::character,   "\\return"sv },
                  { 37, 2, token_kind::character,        "\\"sv },
                  { 40, 2, token_kind::character,        "\\"sv },
                  { 43, 2, token_kind::character,        "\\"sv },
                  { 46, 2, token_kind::character,        "\\"sv },
                  { 49, 6, token_kind::character,    "\\space"sv }
          }));
        }
        ...
        SUBCASE("Hex unicode")
        {
          ...
          CHECK(tokens
                == make_tokens({
                  {   0, 6, token_kind::character, "\\u00a0"sv },
                  {   7, 6, token_kind::character, "\\u1680"sv },
                  {  14, 6, token_kind::character, "\\u2000"sv },
                  {  21, 6, token_kind::character, "\\u2001"sv },
                  {  28, 6, token_kind::character, "\\u2002"sv },
                  {  35, 6, token_kind::character, "\\u2003"sv },
                  {  42, 6, token_kind::character, "\\u2004"sv },
                  {  49, 6, token_kind::character, "\\u2005"sv },
                  {  56, 6, token_kind::character, "\\u2006"sv },
                  {  63, 6, token_kind::character, "\\u2007"sv },
                  {  70, 6, token_kind::character, "\\u2008"sv },
                  {  77, 6, token_kind::character, "\\u2009"sv },
                  {  84, 6, token_kind::character, "\\u200a"sv },
                  {  91, 6, token_kind::character, "\\u2028"sv },
                  {  98, 6, token_kind::character, "\\u2029"sv },
                  { 105, 6, token_kind::character, "\\u202f"sv },
                  { 112, 6, token_kind::character, "\\u205f"sv },
    ...
    TEST_CASE("String")
    {
      ...
      SUBCASE("With escapes")
      {
        ...
        native_vector<jtl::result<token, error_ref>> const tokens2(q.begin(), q.end());
        CHECK(tokens2
              == make_tokens({
