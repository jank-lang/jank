...
#define string_DISABLE_SSO true
#else
#define string_DISABLE_SSO false
#endif
...
class string_core {
 public:
  ...
  string_core(
      ...
      const size_t size,
      bool disableSSO = string_DISABLE_SSO) {
  ...
  FOLLY_NOINLINE
  void reserve(size_t minCapacity, bool disableSSO = string_DISABLE_SSO) {
    FOLLY_PUSH_WARNING
 ...
 private:
  ...
  struct RefCounted {
    ...
    static RefCounted* create(size_t* size) {
      ...
      const size_t allocSize = folly::goodMallocSize(capacityBytes);
      auto result = static_cast<RefCounted*>(folly::checkedMalloc(allocSize));
    ...
    static RefCounted* reallocate(
      ...
      const size_t allocNewCapacity = folly::goodMallocSize(capacityBytes);
  ...
  enum class Category : category_type {
    isSmall = 0,
    isMedium = folly::kIsLittleEndian ? 0x80 : 0x2,
    isLarge = folly::kIsLittleEndian ? 0x40 : 0x1,
  };
  ...
  struct MediumLarge {
    ...
    size_t capacity() const {
      return folly::kIsLittleEndian ? capacity_ & capacityExtractMask : capacity_ >> 2;
    }

    void setCapacity(size_t cap, Category cat) {
      capacity_ = folly::kIsLittleEndian
  ...
  constexpr static uint8_t categoryExtractMask = folly::kIsLittleEndian ? 0xC0 : 0x3;
  constexpr static size_t kCategoryShift = (sizeof(size_t) - 1) * 8;
  constexpr static size_t capacityExtractMask = folly::kIsLittleEndian
  ...
  size_t smallSize() const {
    ...
    constexpr auto shift = folly::kIsLittleEndian ? 0 : 2;
...
template <class Char>
FOLLY_NOINLINE void string_core<Char>::copyLarge(const string_core& rhs) {
...
template <class Char>
inline void string_core<Char>::initSmall(
  ...
  static_assert(
      sizeof(Char*) == sizeof(size_t), "string size assumption violation");
...
template <class Char>
FOLLY_NOINLINE void string_core<Char>::initMedium(
    const Char* const data, const size_t size) {
  // Medium strings are allocated normally. Don't forget to
  // allocate one extra Char for the terminating null.
  auto const allocSize = folly::goodMallocSize((1 + size) * sizeof(Char));
  ml_.data_ = static_cast<Char*>(folly::checkedMalloc(allocSize));
...
template <class Char>
FOLLY_NOINLINE void string_core<Char>::initLarge(
    const Char* const data, const size_t size) {
...
template <class Char>
FOLLY_NOINLINE void string_core<Char>::unshare(size_t minCapacity) {
...
template <class Char>
FOLLY_NOINLINE void string_core<Char>::reserveLarge(size_t minCapacity) {
  assert(category() == Category::isLarge);
  if (RefCounted::refs(ml_.data_) > 1) { // Ensure unique
...
template <class Char>
FOLLY_NOINLINE void string_core<Char>::reserveMedium(
  ...
  if (minCapacity <= maxMediumSize) {
    ...
    size_t capacityBytes = folly::goodMallocSize((1 + minCapacity) * sizeof(Char));
    // Also copies terminator.
...
template <class Char>
FOLLY_NOINLINE void string_core<Char>::reserveSmall(
  ...
  if (!disableSSO && minCapacity <= maxSmallSize) {
    ...
    auto const allocSizeBytes =
        folly::goodMallocSize((1 + minCapacity) * sizeof(Char));
    auto const pData = static_cast<Char*>(folly::checkedMalloc(allocSizeBytes));
...
template <class Char>
inline Char* string_core<Char>::expandNoinit(
    const size_t delta,
    bool expGrowth, /* = false */
    bool disableSSO /* = string_DISABLE_SSO */) {
...
class basic_string {
  ...
  struct Invariant {
    ...
    explicit Invariant(const basic_string& s) noexcept : s_(s) {
      assert(s_.isSane());
    }
 ...
 public:
  ...
  explicit basic_string(const A&) noexcept {}
  ...
  /* implicit */ basic_string(const std::basic_string<E, T, A2>& str)
      : store_(str.data(), str.size()) {}

  // This is defined for compatibility with std::string_view
  /* implicit */ basic_string(const std::string_view& str)
      : store_(str.data(), str.size()) {}
  ...
  FOLLY_NOINLINE
  /* implicit */ basic_string(const value_type* s, const A& /*a*/ = A())
      : store_(s, traitsLength(s)) {}

  FOLLY_NOINLINE
  basic_string(const value_type* s, size_type n, const A& /*a*/ = A())
      : store_(s, n) {}

  FOLLY_NOINLINE
  basic_string(size_type n, value_type c, const A& /*a*/ = A()) {
    auto const pData = store_.expandNoinit(n);
    string_detail::podFill(pData, pData + n, c);
  }

  template <class InIt>
  FOLLY_NOINLINE basic_string(
      InIt begin,
      InIt end,
  ...
  FOLLY_NOINLINE
  basic_string(const value_type* b, const value_type* e, const A& /*a*/ = A())
  ...
  template <typename A2>
  basic_string& operator=(const std::basic_string<E, T, A2>& rhs) {
  ...
  typename std::enable_if<
      std::is_convertible<
          TP,
          typename basic_string<E, T, A, Storage>::value_type>::value &&
          !std::is_same<
              typename std::decay<TP>::type,
              typename basic_string<E, T, A, Storage>::value_type>::value,
      basic_string<E, T, A, Storage>&>::type
  ...
  basic_string& append(
      const basic_string& str, const size_type pos, size_type n);
  ...
  basic_string& assign(ItOrLength first_or_n, ItOrChar last_or_c) {
  ...
  basic_string& insert(
      size_type pos1, const basic_string& str, size_type pos2, size_type n) {
 ...
 public:
  friend inline istream_type& getline(
      istream_type& is, basic_string& str, value_type delim) {
    return str.getlineImpl(is, delim);
  }

  friend inline istream_type& getline(istream_type& is, basic_string& str) {
 ...
 public:
  ...
  jank::native_integer to_hash() const
  {
    if(hash != 0)
    { return hash; }
    ...
    auto const finish(end());
    jank::native_integer h{};
 ...
 private:
  // Data
  Storage store_;
  mutable jank::native_integer hash{};
};

template <typename E, class T, class A, class S>
FOLLY_NOINLINE typename basic_string<E, T, A, S>::size_type
basic_string<E, T, A, S>::traitsLength(const value_type* s) {
  return s ? traits_type::length(s)
           : (folly::throw_exception<std::logic_error>(
                  "basic_string: null pointer initializer not valid"),
              0);
}

template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S>& basic_string<E, T, A, S>::operator=(
...
template <typename E, class T, class A, class S>
inline void basic_string<E, T, A, S>::resize(
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S>& basic_string<E, T, A, S>::append(
    const basic_string& str) {
#ifndef NDEBUG
...
template <typename E, class T, class A, class S>
FOLLY_NOINLINE basic_string<E, T, A, S>& basic_string<E, T, A, S>::append(
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S>& basic_string<E, T, A, S>::assign(
    const basic_string& str, const size_type pos, size_type n) {
  const size_type sz = str.size();
  enforce<std::out_of_range>(pos <= sz, "");
  procrustes(n, sz - pos);
...
template <typename E, class T, class A, class S>
FOLLY_NOINLINE basic_string<E, T, A, S>& basic_string<E, T, A, S>::assign(
...
template <typename E, class T, class A, class S>
inline typename basic_string<E, T, A, S>::istream_type&
basic_string<E, T, A, S>::getlineImpl(istream_type& is, value_type delim) {
  Invariant checker(*this);
...
template <typename E, class T, class A, class S>
inline typename basic_string<E, T, A, S>::size_type
basic_string<E, T, A, S>::find(
...
template <typename E, class T, class A, class S>
inline typename basic_string<E, T, A, S>::iterator
basic_string<E, T, A, S>::insertImplDiscr(
    const_iterator i, size_type n, value_type c, std::true_type) {
  Invariant checker(*this);
...
template <class FwdIterator>
inline typename basic_string<E, T, A, S>::iterator
basic_string<E, T, A, S>::insertImpl(
    const_iterator i,
    FwdIterator s1,
    FwdIterator s2,
    std::forward_iterator_tag) {
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S>& basic_string<E, T, A, S>::replaceImplDiscr(
    iterator i1,
    iterator i2,
    const value_type* s,
    size_type n,
...
template <typename E, class T, class A, class S>
template <class FwdIterator>
inline bool basic_string<E, T, A, S>::replaceAliased(
    iterator i1, iterator i2, FwdIterator s1, FwdIterator s2, std::true_type) {
  std::less_equal<const value_type*> le{};
...
template <typename E, class T, class A, class S>
template <class FwdIterator>
inline void basic_string<E, T, A, S>::replaceImpl(
    iterator i1,
    iterator i2,
    FwdIterator s1,
    FwdIterator s2,
...
template <typename E, class T, class A, class S>
inline typename basic_string<E, T, A, S>::size_type
basic_string<E, T, A, S>::find_first_of(
    const value_type* s, size_type pos, size_type n) const {
  if (pos > length() || n == 0) {
    return npos;
  }
...
template <typename E, class T, class A, class S>
inline typename basic_string<E, T, A, S>::size_type
basic_string<E, T, A, S>::find_last_of(
    const value_type* s, size_type pos, size_type n) const {
  if (!empty() && n > 0) {
...
template <typename E, class T, class A, class S>
inline typename basic_string<E, T, A, S>::size_type
basic_string<E, T, A, S>::find_first_not_of(
    const value_type* s, size_type pos, size_type n) const {
  if (pos < length()) {
    const_iterator i(begin() + pos), finish(end());
    for (; i != finish; ++i) {
...
template <typename E, class T, class A, class S>
inline typename basic_string<E, T, A, S>::size_type
basic_string<E, T, A, S>::find_last_not_of(
    const value_type* s, size_type pos, size_type n) const {
  if (!this->empty()) {
...
// C++11 21.4.8.1/1
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    const basic_string<E, T, A, S>& lhs,
    const basic_string<E, T, A, S>& rhs) {
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    basic_string<E, T, A, S>&& lhs, const basic_string<E, T, A, S>& rhs) {
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    const basic_string<E, T, A, S>& lhs, basic_string<E, T, A, S>&& rhs) {
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    basic_string<E, T, A, S>&& lhs, basic_string<E, T, A, S>&& rhs) {
  return std::move(lhs.append(rhs));
}
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    const E* lhs, const basic_string<E, T, A, S>& rhs) {
  //
  basic_string<E, T, A, S> result;
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    const E* lhs, basic_string<E, T, A, S>&& rhs) {
  //
  const auto len = basic_string<E, T, A, S>::traits_type::length(lhs);
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    E lhs, const basic_string<E, T, A, S>& rhs) {
  basic_string<E, T, A, S> result;
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    E lhs, basic_string<E, T, A, S>&& rhs) {
  //
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    const basic_string<E, T, A, S>& lhs, const E* rhs) {
  typedef typename basic_string<E, T, A, S>::size_type size_type;
  typedef typename basic_string<E, T, A, S>::traits_type traits_type;
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    basic_string<E, T, A, S>&& lhs, const E* rhs) {
  //
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    const basic_string<E, T, A, S>& lhs, E rhs) {
  basic_string<E, T, A, S> result;
...
template <typename E, class T, class A, class S>
inline basic_string<E, T, A, S> operator+(
    basic_string<E, T, A, S>&& lhs, E rhs) {
  //
  return std::move(lhs += rhs);
}
...
template <typename E, class T, class A, class S>
inline bool operator==(
    const typename basic_string<E, T, A, S>::value_type* lhs,
    const basic_string<E, T, A, S>& rhs) {
  return rhs == lhs;
}
...
inline bool operator==(
    const basic_string<E, T, A, S>& lhs,
    const typename basic_string<E, T, A, S>::value_type* rhs) {
...
inline std::basic_istream<
    typename basic_string<E, T, A, S>::value_type,
...
constexpr typename basic_string<E1, T, A, S>::size_type
    basic_string<E1, T, A, S>::npos;
...
// string is relocatable
template <class T, class R, class A, class S>
FOLLY_ASSUME_RELOCATABLE(basic_string<T, R, A, S>);
...
inline std::string toStdString(const folly::string& s) {
...
#define FOLLY_string_HASH1(T)                                        \
  template <>                                                          \
  struct hash<::folly::basic_string<T>> {                            \
    size_t operator()(const ::folly::basic_string<T>& s) const {     \
      return ::folly::hash::fnv32_buf(s.data(), s.size() * sizeof(T)); \
    }                                                                  \
  };

// The C++11 standard says that these four are defined for basic_string
#define FOLLY_string_HASH      \
  FOLLY_string_HASH1(char)     \
  FOLLY_string_HASH1(char16_t) \
  FOLLY_string_HASH1(char32_t) \
  FOLLY_string_HASH1(wchar_t)
...
#undef FOLLY_string_HASH
#undef FOLLY_string_HASH1

FOLLY_POP_WARNING

#undef string_DISABLE_SSO
...
template <>
struct IsSomeString<string> : std::true_type {};
} // namespace folly

template <>
struct fmt::formatter<folly::string> : private formatter<fmt::string_view> {
  using formatter<fmt::string_view>::parse;

  template <typename Context>
  typename Context::iterator format(
      const folly::string& s, Context& ctx) const {
