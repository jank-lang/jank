  ...
  return visit_object(
      [&](auto const typed_s) -> size_t {
        ...
        if constexpr (std::same_as<T, obj::nil>) {
          ...
          size_t length{0};
          for (auto i(typed_s->fresh_seq()); i != nullptr && length < max;
               i = i->next_in_place()) {
            ++length;
          }
          return length;
          ...
          throw std::runtime_error{
              fmt::format("not seqable: {}", typed_s->to_string())};
        }
      },
      s);
}
...
native_bool is_nil(object_ptr const o) { return (o == obj::nil::nil_const()); }
native_bool is_some(object_ptr const o) { return (o != obj::nil::nil_const()); }
          ...
          return visit_object(
              [&](auto const typed_keys) -> object_ptr {
                ...
                if constexpr (behavior::seqable<T>) {
                  ...
                  for (auto seq(typed_keys->fresh_seq()); seq != nullptr;
                       seq = seq->next_in_place()) {
                    ret = get(ret, seq->first());
                  }
                  return ret;
                } else {
                  throw std::runtime_error{
                      fmt::format("not seqable: {}", typed_keys->to_string())};
                }
              },
              keys);
        } else {
