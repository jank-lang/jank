...
(defmethod add-explicit-returns :if-expression [item scope]
  ...
  (let [then-body (second (add-explicit-returns [:body (second (nth item 2))]
                                                scope))
        else-body (second (add-explicit-returns [:body (second (nth item 3))]
                                                scope))]
    ...
    (let [then-type (expression/realize-type
                      (add-explicit-returns then-body scope)
                      scope)
          else-type (expression/realize-type
                      (add-explicit-returns else-body scope)
                      scope)]
      (type-assert (= then-type else-type)
                   "incompatible if then/else types")
      (update-in
        (update-in item [2] (fn [_] [:then [:return then-body]]))
        [3]
        (fn [_] [:else [:return else-body]])))))
